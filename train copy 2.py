...
# ======================================== 读入df_train =====================================
""" 
将数据从物理硬盘中取出, 存入内存中, 命名为df_train, 
对于读入进来的数据不同列, 进行一个手动设置类型, 比如int等
对于深度为0的特征列, 这类特征有一个特点, 就是一个case_id只1个样本
对于深度为1、2的特征列, 这类特征1个case_id对应多个样本, 所以要进行处理, 比如选择max, 在多个样本中只选中最大值
然后将深度为1、2、3的特征列做一个合并
然后对合并的数据用filter_cols进行一个数据过滤, 比如去掉null比较多的列
最后为了节省内存, 在reduce_memory_usage中, 根据不同列的最值, 二次手动设置类型
"""
...
# ======================================== 导入配置 =====================================
""" 定义一些必要的对象, 比如device、k折交叉验证对象cv等 """
...
# ======================================== 特征列分类 =====================================
""" 不同模型所用到的不同特征列, 都有记录, 对df_train进行一个划分, 不同数据用来训练不同的模型 """

...
for idx_train, idx_valid in cv.split(df_train, y, groups=weeks): # 5折，循环5次
    ...
    train_loader = DataLoader(train_set, batch_size=15000, shuffle=True, num_workers=7, drop_last=True)
    valid_set = MarketDataset(valid_preds, y_valid)
    valid_loader = DataLoader(valid_set, batch_size=15000, shuffle=False, num_workers=7, drop_last=True)
